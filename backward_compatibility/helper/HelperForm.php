 ...
 * @since 1.5.0
 */
if (!class_exists('HelperFormCore', false)) {
    ...
    class HelperFormCore extends Helper {
        ...
        public function generate() {
            ...
            if (is_null($this->submit_action)) $this->submit_action = 'submitAdd' . $this->table;
            ...
            foreach ($this->fields_form as $fieldset_key => &$fieldset)
                if (isset($fieldset['form']['input'])) foreach ($fieldset['form']['input'] as $key => &$params) {
                        // If the condition is not met, the field will not be displayed
                        if (isset($params['condition']) && !$params['condition']) unset($this->fields_form[$fieldset_key]['form']['input'][$key]);
                        switch ($params['type']) {
                            case 'categories':
                                if ($categories) {
                                    ...
                                    if (isset($params['use_search']) && $params['use_search']) $this->context->controller->addJS(_PS_JS_DIR_ . 'jquery/plugins/autocomplete/jquery.autocomplete.js');
                                    $categories = false;
                                }
                                break;
                            ...
                            case 'shop' :
                                ...
                                if (Shop::getTotalShops(false) == 1) unset($this->fields_form[$fieldset_key]['form']['input'][$key]);
                                break;
                        }
                    }
        ...
        public function getFieldsRequired() {
            foreach ($this->fields_form as $fieldset)
                if (isset($fieldset['form']['input'])) foreach ($fieldset['form']['input'] as $input)
                        if (array_key_exists('required', $input) && $input['required'] && $input['type'] != 'radio') return true;
...
if (!class_exists('HelperForm', false)) {
